...
import { ASelect, AForm, AFormItem, type AFormInstance } from '@common/shared/ui'
...
import { InvoiceOttobockFormValidationSchema } from '../../model'
import { invoiceOttobockMappers } from '../../lib'
import type { SelectOption } from '@rupoi/shared/model'

interface Props {
  data?: InvoiceOttobockDTO | null
  submitText?: string
  disabled?: boolean
}
...
const emit = defineEmits<{
  submit: [value: InvoiceOttobockFormValues]
}>()

const formRef = ref<AFormInstance<InvoiceOttobockFormValues>>()

const model = ref<InvoiceOttobockFormValues>(
  props.data ? invoiceOttobockMappers.dtoToFormValues(props.data) : {
    number: null,
    requiredCopy: null,
...
watch(
  ...
  (data) => {
    if (data) {
      model.value = invoiceOttobockMappers.dtoToFormValues(data)
    }
  }
)
...
<template>
  <AForm
    ref="formRef"
    :model="model"
    :rules="InvoiceOttobockFormValidationSchema"
  >
    <div class="row g-3">
      <!-- Номер накладной -->
