...
import { InvoiceReadyMadeFormValidationSchema } from '../../model'
import { invoiceReadyMadeMappers } from '../../lib'
import type { SelectOption } from '@rupoi/shared/model'

interface Props {
  data?: InvoiceReadyMadeDTO | null
  submitText?: string
  disabled?: boolean
}
...
const emit = defineEmits<{
  submit: [value: InvoiceReadyMadeFormValues]
}>()

const formRef = ref<AFormInstance<InvoiceReadyMadeFormValues>>()

const model = ref<InvoiceReadyMadeFormValues>(
  props.data ? invoiceReadyMadeMappers.dtoToFormValues(props.data) : {
    number: null,
    requiredCopy: null,
...
watch(
  ...
  (data) => {
    if (data) {
      model.value = invoiceReadyMadeMappers.dtoToFormValues(data)
    }
  }
)
...
<template>
  <AForm
    ref="formRef"
    :model="model"
    :rules="InvoiceReadyMadeFormValidationSchema"
  >
    <div class="row g-3">
      <!-- Номер накладной -->
