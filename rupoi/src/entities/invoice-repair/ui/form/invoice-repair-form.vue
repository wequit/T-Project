...
import { InvoiceRepairFormValidationSchema } from '../../model'
import { invoiceRepairMappers } from '../../lib'
import type { SelectOption } from '@rupoi/shared/model'

interface Props {
  data?: InvoiceRepairDTO | null
  submitText?: string
  disabled?: boolean
}
...
const emit = defineEmits<{
  submit: [value: InvoiceRepairFormValues]
}>()

const formRef = ref<AFormInstance<InvoiceRepairFormValues>>()

const model = ref<InvoiceRepairFormValues>(
  props.data ? invoiceRepairMappers.dtoToFormValues(props.data) : {
    number: null,
    requiredCopy: null,
...
watch(
  ...
  (data) => {
    if (data) {
      model.value = invoiceRepairMappers.dtoToFormValues(data)
    }
  }
)
...
<template>
  <AForm
    ref="formRef"
    :model="model"
    :rules="InvoiceRepairFormValidationSchema"
  >
    <div class="row g-3">
      <!-- Номер накладной -->
