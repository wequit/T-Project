...
import type { InvoiceRepairDTO } from '@rupoi/entities/invoice-repair/model/types.ts'
...
interface InvoiceRepairTableProps {
  data: InvoiceRepairDTO[]
  isLoading?: boolean
  isError?: boolean
}

const props = withDefaults(defineProps<InvoiceRepairTableProps>(), {
  isLoading: false,
  isError: false,
})
...
const columns = computed<ColumnDef<InvoiceRepairDTO, unknown>[]>(() => [
  {
    id: 'number',
    header: '№ накладной',
    enableSorting: true,
  ...
  },
  {
    ...
    enableSorting: false,
    cell: (ctx) => {
      const cost = ctx.row.original.order?.repairCost
