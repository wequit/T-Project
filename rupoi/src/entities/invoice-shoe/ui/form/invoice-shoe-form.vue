...
import { InvoiceShoeFormValidationSchema } from '../../model'
import { invoiceShoeMappers } from '../../lib'
import type { SelectOption } from '@rupoi/shared/model'

interface Props {
  data?: InvoiceShoeDTO | null
  submitText?: string
  disabled?: boolean
}
...
const emit = defineEmits<{
  submit: [value: InvoiceShoeFormValues]
}>()

const formRef = ref<AFormInstance<InvoiceShoeFormValues>>()

const model = ref<InvoiceShoeFormValues>(
  props.data ? invoiceShoeMappers.dtoToFormValues(props.data) : {
    number: null,
    requiredCopy: null,
...
watch(
  ...
  (data) => {
    if (data) {
      model.value = invoiceShoeMappers.dtoToFormValues(data)
    }
  }
)
...
<template>
  <AForm
    ref="formRef"
    :model="model"
    :rules="InvoiceShoeFormValidationSchema"
  >
    <div class="row g-3">
      <!-- Номер накладной -->
