...
import type { OrderProsthesisDTO } from '@rupoi/entities/order-prosthesis/model/types.ts'
...
interface OrderProsthesisTableProps {
  data: OrderProsthesisDTO[]
  isLoading?: boolean
  isError?: boolean
}

const props = withDefaults(defineProps<OrderProsthesisTableProps>(), {
  isLoading: false,
  isError: false,
})
...
const columns = computed<ColumnDef<OrderProsthesisDTO, unknown>[]>(() => [
  {
    id: 'orderNumber',
    header: '№ заказа',
    enableSorting: true,
  ...
  { id: 'productType', header: 'Тип изделия', accessorKey: 'productTypeRef.nameRu', enableSorting: true },
  {
    id: 'priority',
    header: 'Приоритет',
