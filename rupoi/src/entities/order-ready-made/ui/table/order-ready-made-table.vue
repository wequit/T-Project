...
import type { OrderReadyMadeDTO } from '@rupoi/entities/order-ready-made/model/types.ts'
...
interface OrderReadyMadeTableProps {
  data: OrderReadyMadeDTO[]
  isLoading?: boolean
  isError?: boolean
}

const props = withDefaults(defineProps<OrderReadyMadeTableProps>(), {
  isLoading: false,
  isError: false,
})
...
const columns = computed<ColumnDef<OrderReadyMadeDTO, unknown>[]>(() => [
  {
    id: 'orderNumber',
    header: '№ заказа',
    enableSorting: true,
