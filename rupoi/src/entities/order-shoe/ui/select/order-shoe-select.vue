...
import { useOrderShoeList } from '../../model/queries'
import type { ID } from '@rupoi/shared/model'
import type { OrderShoeListParams } from '../../model/types'
...
// Query params for order shoe list
const orderListParams = ref<OrderShoeListParams>({
  query: undefined,
...
const orderListQuery = useOrderShoeList(orderListParams)
...
const options = computed<SearchBoxOption<ID>[]>(() => {
  return (orderListQuery.data.value?.content ?? []).map(order => {
    ...
    const model = order.modelRef?.nameRu || 'Не указана'

    return {
      value: order.id,
      label: `${orderNumber} • ${personName}`,
      subLabel: `Модель: ${model}`,
      data: order,
    }
  })
})
