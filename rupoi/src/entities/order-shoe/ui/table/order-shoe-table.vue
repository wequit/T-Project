...
import type { OrderShoeDTO } from '@rupoi/entities/order-shoe/model/types.ts'
...
interface OrderShoeTableProps {
  data: OrderShoeDTO[]
  isLoading?: boolean
  isError?: boolean
}

const props = withDefaults(defineProps<OrderShoeTableProps>(), {
  isLoading: false,
  isError: false,
})
...
const columns = computed<ColumnDef<OrderShoeDTO, unknown>[]>(() => [
  {
    id: 'orderNumber',
    header: '№ заказа',
    enableSorting: true,
  ...
  { id: 'createdAt', header: 'Дата', accessorKey: 'createdAt', enableSorting: true, cell: (ctx) => formatDate(ctx.getValue() as string) },
  {
    id: 'person',
    header: 'Клиент',
    enableSorting: false,
  ...
  { id: 'modelId', header: 'Модель', accessorKey: 'modelRef.nameRu', enableSorting: true },
  {
    id: 'priority',
    header: 'Приоритет',
