...
import { OrderReadyMadeForm } from '@rupoi/entities/order-ready-made'
import { useOrderReadyMadeSave } from '../model/use-order-ready-made-save'
...
const emit = defineEmits<{
  submit: [value: OrderReadyMadeDTO | null]
  success: [value: OrderReadyMadeDTO | null]
}>()
...
const {
  ...
  submit: handleSubmit,
  orderReadyMade,
} = useOrderReadyMadeSave({
  orderReadyMadeId: toRef(props, 'orderReadyMadeId'),
  personId: computed(() => selectedPersonId.value ?? props.personId ?? null),
  onSuccess: (orderReadyMadeId) => {
    router.push({
      name: RouteConfig.Order.readyMade.edit.name,
      params: { id: orderReadyMadeId }
    })
  }
})
...
const personFullName = computed(() => {
  if (!orderReadyMade.value?.person) return ''
  const { lastName, firstName, patronymic } = orderReadyMade.value.person
  return `${lastName} ${firstName} ${patronymic}`
})

async function onSubmit(formValues: OrderReadyMadeFormValues): Promise<void> {
...
<template>
  <div class="order-ready-made-save-wrapper">
