...
import {
  useInvoiceRepair,
  InvoiceRepairInfoCard,
} from '@rupoi/entities/invoice-repair'
import {
...
function handleDelete() {
  if (confirm(`Вы уверены, что хотите удалить накладную № ${invoiceRepair.value?.number || invoiceId.value}?`)) {
    deleteInvoiceRepairMutation.mutateAsync(invoiceId.value).then(() => {
      router.push({ name: RouteConfig.Invoice.repair.list.name })
    })
  }
}
...
const invoiceTitle = computed(() => {
  if (!invoiceRepair.value) return ''
  return `Накладная № ${invoiceRepair.value.number || invoiceId.value}`
})

const orderRepair = computed(() => invoiceRepair.value?.order ?? null)

const canModify = computed(() => invoiceRepair.value?.status === InvoiceStatus.ACTIVE && canEditInvoice.value)

const canDelete = computed(() => invoiceRepair.value?.status === InvoiceStatus.ACTIVE && canDeleteInvoice.value)
</script>
...
<template>
  <div>
          ...
          <div class="card-body py-3">
            <div class="d-flex justify-content-between align-items-center">
              <div class="d-flex align-items-center gap-3">
                <BaseStatusTag v-if="invoiceRepair" :status="invoiceRepair.status" size="sm" />
              ...
              <div class="d-flex justify-content-between align-items-center gap-2">
                ...
                <div class="d-flex gap-2">
                  ...
                  <AButton
                    v-if="invoiceRepair && canDelete"
                    color="danger"
                    size="sm"
                    :disabled="deleteInvoiceRepairMutation.isPending.value"
                    @click="handleDelete"
                  >
    ...
    <template v-if="invoiceRepair">
      <div class="row">
        <div class="col-12">
          <BaseTabs v-model="activeTab" :tabs="tabs" variant="pills" size="md">
            <template #default="{ activeTab: currentTab }">
              <InvoiceRepairInfoCard v-if="currentTab === 'invoice'" :data="invoiceRepair" />
              <OrderRepairCommonInfoCard v-if="currentTab === 'order' && orderRepair" :data="orderRepair" />
              <OrderRepairFittingsCard v-if="currentTab === 'fittings' && orderRepair" :data="orderRepair" />
              <OrderRepairMaterialsCard v-if="currentTab === 'materials' && orderRepair" :data="orderRepair" />
            </template>
          </BaseTabs>
        </div>
      </div>
