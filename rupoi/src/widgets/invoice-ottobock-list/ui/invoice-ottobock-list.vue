...
import {
  InvoiceOttobockTable,
  InvoiceOttobockFilterForm,
  useInvoiceOttobock,
  type InvoiceOttobockListParams,
  type InvoiceOttobockDTO,
  type InvoiceOttobockFilterFormValues,
...
const { filter, updateFilter } = useFilter<InvoiceOttobockListParams>({
  initFilter: () => ({
...
const filterFormValues = computed<InvoiceOttobockFilterFormValues>(() => ({
...
function handleSearch(data: InvoiceOttobockFilterFormValues) {
...
function handleReset(data: InvoiceOttobockFilterFormValues) {
  const statusValue = data.status as InvoiceStatus[]
  const priorityValue = data.priority as OrderPriority | undefined
...
function handleView(invoice: InvoiceOttobockDTO) {
  router.push({
...
function handleEdit(invoice: InvoiceOttobockDTO) {
  router.push({
...
const {
  invoiceOttobockList: data,
  isLoading,
  listQuery,
  deleteInvoiceOttobockMutation
} = useInvoiceOttobock({ listParams: filter })
const isError = computed(() => listQuery?.isError.value ?? false)

function handleDelete(invoice: InvoiceOttobockDTO) {
  if (confirm(`Вы уверены, что хотите удалить накладную № ${invoice.number || invoice.id}?`)) {
    deleteInvoiceOttobockMutation.mutateAsync(invoice.id).then(() => {
      listQuery?.refetch()
    })
  }
}
...
function canModify(invoice: InvoiceOttobockDTO) {
  return invoice.status === InvoiceStatus.ACTIVE && canEditInvoice.value
}

function canDelete(invoice: InvoiceOttobockDTO) {
  return invoice.status === InvoiceStatus.ACTIVE && canDeleteInvoice.value
}
</script>

<template>
  <div class="invoice-ottobock-list">
    <div class="invoice-ottobock-list__filters mb-4">
      <InvoiceOttobockFilterForm
    ...
    <div class="invoice-ottobock-list__table mb-4">
      <div class="card">
          ...
          >
            <template #actions="{ item }">
              <div class="d-flex gap-2 justify-content-center">
                <APopover v-if="canViewInvoice" title="Просмотр накладной" trigger="hover">
                  <AButton color="primary" size="sm" icon @click="handleView(item)">
                    <i class="fas fa-eye"></i>
                  </AButton>
                </APopover>
                <APopover v-if="canModify(item)" title="Редактирование накладной" trigger="hover">
                  <AButton
                    color="warning"
                    size="sm"
                    icon
                ...
                <APopover v-if="canDelete(item)" title="Удалить накладную" trigger="hover">
                  <AButton
                    color="danger"
                    size="sm"
                    icon
    ...
    <div v-if="showPagination" class="invoice-ottobock-list__pagination d-flex justify-content-end">
      <BasePagination
    ...
    <div v-if="$slots.footer" class="invoice-ottobock-list__footer mt-4">
      <slot name="footer" />
    </div>
  </div>
</template>
