...
import {
  InvoiceProsthesisTable,
  InvoiceProsthesisFilterForm,
  useInvoiceProsthesis,
  type InvoiceProsthesisListParams,
  type InvoiceProsthesisDTO,
  type InvoiceProsthesisFilterFormValues,
...
// Filter logic
const { filter, updateFilter } = useFilter<InvoiceProsthesisListParams>({
  initFilter: () => ({
...
const filterFormValues = computed<InvoiceProsthesisFilterFormValues>(() => ({
...
function handleSearch(data: InvoiceProsthesisFilterFormValues) {
...
function handleReset(data: InvoiceProsthesisFilterFormValues) {
  const statusValue = data.status as InvoiceStatus[]
  const priorityValue = data.priority as OrderPriority | undefined
...
function handleView(invoice: InvoiceProsthesisDTO) {
  router.push({
...
function handleEdit(invoice: InvoiceProsthesisDTO) {
  router.push({
...
// Loading Data
const {
  invoiceProsthesisList: data,
  isLoading,
  listQuery,
  deleteInvoiceProsthesisMutation
} = useInvoiceProsthesis({ listParams: filter })
const isError = computed(() => listQuery?.isError.value ?? false)

// Delete handler
function handleDelete(invoice: InvoiceProsthesisDTO) {
  if (confirm(`Вы уверены, что хотите удалить накладную № ${invoice.number || invoice.id}?`)) {
    deleteInvoiceProsthesisMutation.mutateAsync(invoice.id).then(() => {
...
function canModify(invoice: InvoiceProsthesisDTO) {
  return invoice.status === InvoiceStatus.ACTIVE && canEditInvoice.value
}

function canDelete(invoice: InvoiceProsthesisDTO) {
  return invoice.status === InvoiceStatus.ACTIVE && canDeleteInvoice.value
}
</script>

<template>
  <div class="invoice-prosthesis-list">
    <!-- Форма фильтрации -->
    <div class="invoice-prosthesis-list__filters mb-4">
      <InvoiceProsthesisFilterForm
    ...
    <!-- Таблица -->
    <div class="invoice-prosthesis-list__table mb-4">
      <div class="card">
    ...
    <!-- Пагинация -->
    <div v-if="showPagination" class="invoice-prosthesis-list__pagination d-flex justify-content-end">
      <BasePagination
    ...
    <div v-if="$slots.footer" class="invoice-prosthesis-list__footer mt-4">
      <slot name="footer" />
    </div>
  </div>
</template>
