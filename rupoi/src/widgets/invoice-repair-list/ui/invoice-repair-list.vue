...
import {
  InvoiceRepairTable,
  InvoiceRepairFilterForm,
  useInvoiceRepair,
  type InvoiceRepairListParams,
  type InvoiceRepairDTO,
  type InvoiceRepairFilterFormValues,
...
// Filter logic
const { filter, updateFilter } = useFilter<InvoiceRepairListParams>({
  initFilter: () => ({
...
const filterFormValues = computed<InvoiceRepairFilterFormValues>(() => ({
...
function handleSearch(data: InvoiceRepairFilterFormValues) {
...
function handleReset(data: InvoiceRepairFilterFormValues) {
  const statusValue = data.status as InvoiceStatus[]
  const priorityValue = data.priority as OrderPriority | undefined
...
function handleView(invoice: InvoiceRepairDTO) {
  router.push({
...
function handleEdit(invoice: InvoiceRepairDTO) {
  router.push({
...
// Loading Data
const {
  invoiceRepairList: data,
  isLoading,
  listQuery,
  deleteInvoiceRepairMutation
} = useInvoiceRepair({ listParams: filter })
const isError = computed(() => listQuery?.isError.value ?? false)

// Delete handler
function handleDelete(invoice: InvoiceRepairDTO) {
  if (confirm(`Вы уверены, что хотите удалить накладную № ${invoice.number || invoice.id}?`)) {
    deleteInvoiceRepairMutation.mutateAsync(invoice.id).then(() => {
...
function canModify(invoice: InvoiceRepairDTO) {
  return invoice.status === InvoiceStatus.ACTIVE && canEditInvoice.value
}

function canDelete(invoice: InvoiceRepairDTO) {
  return invoice.status === InvoiceStatus.ACTIVE && canDeleteInvoice.value
}
</script>

<template>
  <div class="invoice-repair-list">
    <!-- Форма фильтрации -->
    <div class="invoice-repair-list__filters mb-4">
      <InvoiceRepairFilterForm
    ...
    <!-- Таблица -->
    <div class="invoice-repair-list__table mb-4">
      <div class="card">
    ...
    <!-- Пагинация -->
    <div v-if="showPagination" class="invoice-repair-list__pagination d-flex justify-content-end">
      <BasePagination
    ...
    <div v-if="$slots.footer" class="invoice-repair-list__footer mt-4">
      <slot name="footer" />
    </div>
  </div>
</template>
