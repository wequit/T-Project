...
import {
  OrderRepairTable,
  OrderRepairFilterForm,
  useOrderRepair,
  type OrderRepairListParams,
  type OrderRepairDTO,
  type OrderRepairFilterFormValues,
...
// Filter logic
const { filter, updateFilter } = useFilter<OrderRepairListParams>({
  initFilter: () => ({
...
const filterFormValues = computed<OrderRepairFilterFormValues>(() => ({
  query: filter.value.query ?? null,
  status: filter.value.status ?? [],
  priority: filter.value.priority ?? null,
}))

function handleSearch(data: OrderRepairFilterFormValues) {
...
function handleReset(data: OrderRepairFilterFormValues) {
  const statusValue = data.status as OrderStatus[]
  const priorityValue = data.priority as OrderPriority | undefined
...
function handleView(order: OrderRepairDTO) {
  router.push({
...
function handleEdit(order: OrderRepairDTO) {
  router.push({
...
// Loading Data
const {
  orderRepairList: data,
  isLoading,
  listQuery,
  deleteOrderRepairMutation
} = useOrderRepair({ listParams: filter })
const isError = computed(() => listQuery?.isError.value ?? false)

// Delete handler
function handleDelete(order: OrderRepairDTO) {
  if (confirm(`Вы уверены, что хотите удалить наряд № ${order.orderNumber || order.id}?`)) {
    deleteOrderRepairMutation.mutateAsync(order.id).then(() => {
      listQuery?.refetch()
    })
  }
}
...
function canModify(order: OrderRepairDTO) {
  return order.orderStatus === OrderStatus.NEW && canEditOrder.value
}

function canDelete(order: OrderRepairDTO) {
...
<template>
  <div class="order-repair-list">
    <!-- Форма фильтрации -->
    <div class="order-repair-list__filters mb-4">
      <OrderRepairFilterForm
    ...
    <!-- Таблица -->
    <div class="order-repair-list__table mb-4">
      <div class="card">
    ...
    <!-- Пагинация -->
    <div v-if="showPagination" class="order-repair-list__pagination d-flex justify-content-end">
      <BasePagination
    ...
    <div v-if="$slots.footer" class="order-repair-list__footer mt-4">
      <slot name="footer" />
    </div>
  </div>
</template>
